	...
	var info = document.getElementById('join_form');
	info.onsubmit = function(){
		...
		var nNum = document.getElementById('nationNo').value;
		var phoneNum = document.getElementById('iphoneNo').value;

		alert(`아이디 :${id} 비밀번호: ${pw} 이름: ${name} 생년월일(년${year} ${month} 일${day}) 
			성별: ${sex} 이메일: ${email} 휴대전화( ${nNum} ${phoneNum} )`);
		return false;
		// preventDefault();
		}	
	}
